function loadNotes() {
    ...
    if (noteList) { // Check if the element exists
        noteList.innerHTML += localStorage.getItem('noteList') || '';
        let noteCount = localStorage.getItem('noteCount') || 0;
        let noteCountElement = document.getElementById('noteCount');
        if (noteCountElement) {
            noteCountElement.textContent = `Total Notes: ${noteCount}`;
        }
    } else {
...
// Add a note to localStorage and update the display
function addNote() {
    let note = document.getElementById('note').value;
    localStorage.setItem('note', note);

    // Retrieve the current noteAmount from localStorage, defaulting to 0 if it doesn't exist
    let currentNoteAmount = parseInt(localStorage.getItem('noteAmount')) || 0; 

    // Increment the noteAmount
    currentNoteAmount += 1 * 1000
    // Store the updated noteAmount back into localStorage
